    ...
    for instancia in instancias:
        try:
            ...
            if response.status_code == 200:
                ...
                posts.extend(
                    [t.strip() for t in tweets if len(t.strip()) > 20]
                )
...
def buscar_web_geral(username):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            posts.extend(
                [r.strip() for r in resultados if len(r.strip()) > 30]
            )
    except:
        pass
...
def buscar_todas_plataformas(username):
    ...
    if posts_twitter:
        ...
        plataformas_encontradas.append(
            f'Twitter/X ({len(posts_twitter)} tweets)'
        )
...
def menu():
    ...
    while True:
        ...
        if opcao == '1':
            ...
            print(
                '   e analisa o conteúdo para identificar tendência política'
            )
