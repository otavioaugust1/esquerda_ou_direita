...
def limpar_texto(texto):
    ...
    texto = re.sub(
        r'@(\w+)', '', texto
    )   # Remove menções (vamos analisar separadamente)
        ...
        if eh_rt:
            ...
            for mencao in mencões:
                if mencao in PERFIS_REF_ESQUERDA:
                    score_esq += (
                        3.0  # Peso alto para RT de perfil de referência
                    )
...
def main():
    ...
    print(f"{'='*70}")
    print(
        f"{'USUÁRIO':<20} {'ESQUERDA':<10} {'DIREITA':<10} {'CLASSIFICAÇÃO':<30}"
    )
    print(f"{'-'*70}")

    for res in resultados:
        print(
            f"@{res['usuario']:<18} {res['esquerda']:<9.1f}% {res['direita']:<9.1f}% {res['classificacao']}"
        )
    ...
    with open(
        f"resultados_analise_{datetime.now().strftime('%Y%m%d')}.json",
        'w',
        encoding='utf-8',
    ) as f:
...
def teste_rapido():
    ...
    usuario_teste = input(
        'Digite o @ do usuário para análise (ex: LulaOficial): '
    ).strip()
