        ...
        try:
            ...
            for padrao in padroes:
                ...
                if tweets_encontrados:
                    ...
                    if tweets:
                        print(
                            f'  ✓ Coletados {len(tweets)} tweets de @{usuario}'
                        )
                        return tweets
            ...
            try:
                ...
                if response_rss.status_code == 200:
                    ...
                    if tweets:
                        print(
                            f'  ✓ Coletados {len(tweets)} tweets via RSS de @{usuario}'
                        )
                        return tweets
            except:
                pass
...
def analisar_usuario_completo(usuario):
    ...
    if tweets_analisados < 5:
        ...
        if pct_esq > pct_dir:
            classificacao = (
                f'NEUTRO com Inclinação à ESQUERDA ({pct_esq:.1f}%)'
            )
    ...
    print(
        f'  📈 Esquerda: {pct_esq:.1f}% | Direita: {pct_dir:.1f}% | Neutro: {pct_neutro:.1f}%'
    )
...
def analisar_lista_completa():
    ...
    if resultados:
        ...
        for res in resultados:
            ...
            print(
                f"{emoji} @{res['usuario']:<18} {res['esquerda']:>9.1f}% {res['direita']:>9.1f}% {res['classificacao'][:28]}"
            )
...
def salvar_resultados(resultados):
    ...
    # CSV
    with open(
        f'resultados_reais_{timestamp}.csv', 'w', newline='', encoding='utf-8'
    ) as f:
