...
def buscar_twitter_api_oficial(username):
    ...
    try:
        ...
        response = requests.get(
            url_tweets, headers=headers, params=params, timeout=10
        )
...
def buscar_twitter_nitter_backup(username):
    ...
    posts = []
    instancias = ['nitter.poast.org', 'nitter.privacydev.net', 'nitter.net']
...
def buscar_noticias_sobre_usuario(nome_completo):
    ...
    print(f'\n📰 NOTÍCIAS')
    print(
        f"   Buscando notícias sobre '{nome_completo}'...", end=' ', flush=True
    )
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            for titulo in titulos[1:30]:
                if (
                    len(titulo) > 20
                    and nome_completo.lower() in titulo.lower()
                ):
                    posts.append(
                        {
                            'texto': titulo,
...
def salvar_resultado(resultado, posts):
    ...
    nome_limpo = (
        re.sub(r'[^\w\s-]', '', resultado['nome']).strip().replace(' ', '_')
    )
...
def menu():
    ...
    while True:
        ...
        if tem_twitter_api:
            ...
            print(
                '⚠️  Twitter API: NÃO CONFIGURADA (método backup será usado)'
            )
        ...
        if opcao == '1':
            ...
            try:
                ...
                twitter = input(
                    'Username no Twitter (ou Enter para pular): '
                ).strip()
            except (KeyboardInterrupt, EOFError):
                print('\n❌ Cancelado')
                continue
