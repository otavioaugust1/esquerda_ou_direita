...
def coletar_dados_simulados(usuario):
    ...
    # Perfis de esquerda
    if any(
        palavra in usuario.lower()
        for palavra in ['lula', 'pt', 'boulos', 'haddad', 'gleisi']
    ):
    ...
    # Perfis de direita
        ...
        palavra in usuario.lower()
        for palavra in ['bolsonaro', 'zambelli', 'nikolas', 'mbl', 'mito']
    ):
...
def gerar_relatorio(resultados):
    ...
    # Relatório em CSV
    with open(
        f'relatorio_analise_{timestamp}.csv', 'w', newline='', encoding='utf-8'
    ) as f:
    ...
    # Relatório em JSON
    with open(
        f'detalhes_analise_{timestamp}.json', 'w', encoding='utf-8'
    ) as f:
    ...
    with open(f'resumo_analise_{timestamp}.txt', 'w', encoding='utf-8') as f:
        ...
        for res in resultados:
            ...
            f.write(
                f"  • Hashtags principais: {', '.join(res['hashtags_mais_usadas'])}\n"
            )
        ...
        if len(resultados) > 0:
            media_esq = sum(r['esquerda'] for r in resultados) / len(
                resultados
            )
...
def main():
    ...
    for res in resultados:
        emoji = (
            '🔴'
            if 'ESQUERDA' in res['classificacao']
            else '🔵'
            if 'DIREITA' in res['classificacao']
            else '⚪'
        )
        print(
            f"{emoji} @{res['usuario']:<22} {res['esquerda']:>10.1f}% {res['direita']:>10.1f}% {res['classificacao']:<30}"
        )
...
def teste_rapido():
    ...
    usuario = (
        input('Digite o @ do usuário (ex: LulaOficial): ')
        .strip()
        .replace('@', '')
    )
    ...
    if resultado['hashtags_mais_usadas']:
        print(
            f"  • Hashtags principais: {', '.join(resultado['hashtags_mais_usadas'])}"
        )
